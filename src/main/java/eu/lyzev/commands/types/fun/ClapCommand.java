...
public class ClapCommand implements ServerCommand {
    @Override
    public void performCommand(Member member, TextChannel channel, Message message) {
        ...
        if (message.getContentDisplay().split(" ").length > 1)
            channel.sendMessage(String.join(":clap:", message.getContentDisplay().substring(message.getContentDisplay().split(" ")[0].length() + 1).split(""))).queue();
        else {
            EmbedBuilder eb = DogeAPI.createEmbed();
            eb.setTitle("Doge Clap");
            eb.setDescription("What should I clap for you?\n" + CommandListener.prefix + "clap [text]");
